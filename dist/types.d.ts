...
import { Socket as _Socket1 } from "socket.io-client";
...
declare class Socket extends EventEmitter {
    _socketio?: _Socket1;
    constructor(secure: any, host: string, port: number, path: string, key: string, clientType: PeerOptions["clientType"], pingInterval?: number);
    start(id: string, token: string): Promise<void>;
...
export abstract class DataConnection extends BaseConnection<DataConnectionEvents, DataConnectionErrorType> {
    ...
    protected updateMaximumMessageSize(): Promise<void>;
    /** Called by the Negotiator when the DataChannel is ready. */
